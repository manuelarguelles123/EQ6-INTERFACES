...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const slideInterval = setInterval(showNextSlide, 10000);

    // Navegación manual con las flechas
    if (nextSlideBtn) {
        nextSlideBtn.addEventListener("click", () => {
            clearInterval(slideInterval); // Detener el cambio automático temporalmente
            showNextSlide();
        });
    }
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const closeUserAlertBtn = document.getElementById("close-user-alert");
    const goToLoginBtn = document.getElementById("go-to-login");
    const userButton = document.querySelector(".icon-btn[title='Usuario']");

    // Mostrar el cuadro de alerta al hacer clic en el botón de usuario
    userButton.addEventListener("click", () => {
        userAlertModal.classList.add("active");
    });

    // Cerrar el cuadro de alerta al hacer clic en el botón "Cerrar"
    closeUserAlertBtn.addEventListener("click", () => {
        userAlertModal.classList.remove("active");
    });

    // Redirigir al login al hacer clic en el botón "Ir a Login"
    goToLoginBtn.addEventListener("click", () => {
        window.location.href = "../login.html"; // Ruta relativa al archivo login.html
    });
});